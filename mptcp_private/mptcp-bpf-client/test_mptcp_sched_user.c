...
int main(int argc, char **argv)
{
	...
	if (argc > 2) {
		...
		printf("Please specify the bpf scheduler program and script.\n"
			" e.g.: ./test_mptcp_user  bpf_mptcp_option_scheduler.o my_net_scheduler.sh -q\n");
		exit(1);
	}
	...
	int map_fd = bpf_find_map(__func__, obj, "sched_map");
	if (map_fd < 0)
		goto err;
	...
	rv = bpf_map_update_elem(map_fd, &key, sched, BPF_ANY);
	if (rv != 0) {
