...
# path 1-2, client-to-server
  ...
  # `burst` (in bytes) is mandatory: should be at least = rate (Mbits) / 1000 (Hz)
$NS1 tc qdisc add dev veth1   root tbf rate 20Mbit latency 1ms burst 40000

  # mean (50ms) > delta (40ms) to avoid out-of-order
$NS_BR tc qdisc add dev ethBr2   root handle 1:0    netem delay 50ms 40ms 25% #$loss 0.5%
$NS_BR tc qdisc add dev ethBr2   parent 1:1 pfifo limit 1000


# path 3-4, client-to-server
$NS1 tc qdisc add dev veth3   root tbf rate 20Mbit latency 1ms burst 40000
$NS_BR tc qdisc add dev ethBr4   root handle 1:0    netem delay 50ms 40ms 25% #loss 0.5%
$NS_BR tc qdisc add dev ethBr4   parent 1:1 pfifo limit 1000


# path 2-1, server-to-client
$NS2 tc qdisc add dev veth2  root  tbf rate 20Mbit latency 1ms burst 40000
$NS_BR tc qdisc add dev ethBr1   root handle 1:0    netem delay 50ms 40ms 25% #loss 0.5%
$NS_BR tc qdisc add dev ethBr1   parent 1:1 pfifo limit 1000


# path 4-3, server-to-client
$NS2 tc qdisc add dev veth4   root tbf rate 20Mbit latency 1ms burst 40000
$NS_BR tc qdisc add dev ethBr3   root handle 1:0    netem delay 50ms 40ms 25% #$loss 0.5%
$NS_BR tc qdisc add dev ethBr3   parent 1:1 pfifo limit 1000


$NS1   tc -s qdisc show dev veth1
...
$NS2  curl $serverIP:$serverPort/vmlinux.o  -m 10  -o /dev/null
