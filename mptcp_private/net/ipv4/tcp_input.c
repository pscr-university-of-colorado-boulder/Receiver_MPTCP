...
int sysctl_fast_mptcp_plus_rtt_wnd __read_mostly = 10;//To get the average RTT
int sysctl_fast_mptcp_plus_rtt_thr __read_mostly = 2;//For changes in Network Condition
int sysctl_fast_mptcp_plus_rsv1 __read_mostly = 15;//This is to set the RTT ratio
...
static void tcp_rcv_rtt_update(struct tcp_sock *tp, u32 sample, int win_dep)
{
	...
	//FAST_MPTCP_PLUS
	if((tp->rcv_rtt_est.rtt_us>>3)!=0){
                tp->fast_mptcp_plus.rtt =(tp->rcv_rtt_est.rtt_us>>3);
		if(tp->fast_mptcp_plus.rtt_prev==0){
			tp->fast_mptcp_plus.rtt_prev=tp->fast_mptcp_plus.rtt;
			tp->fast_mptcp_plus.rtt_start_time=cur_time;
			tp->fast_mptcp_plus.rsv1=0;
		}
		if(tp->fast_mptcp_plus.rtt_avg==0){
			tp->fast_mptcp_plus.rtt_avg=tp->fast_mptcp_plus.rtt;
			tp->fast_mptcp_plus.rtt_count++;
		}
		else{
			if(tp->fast_mptcp_plus.rtt_count<sysctl_fast_mptcp_plus_rtt_wnd)
				tp->fast_mptcp_plus.rtt_count++;
			tp->fast_mptcp_plus.rtt_avg-=tp->fast_mptcp_plus.rtt_avg/tp->fast_mptcp_plus.rtt_count;
			tp->fast_mptcp_plus.rtt_avg+=tp->fast_mptcp_plus.rtt/tp->fast_mptcp_plus.rtt_count;
		}
		if(tp->fast_mptcp_plus.rtt>=(sysctl_fast_mptcp_plus_rtt_thr*tp->fast_mptcp_plus.rtt_prev) && tp->fast_mptcp_plus.rtt_prev!=0){ //Sudden change in Network Condition
					tp->fast_mptcp_plus.rtt_min = 0;
					tp->fast_mptcp_plus.rsv1=0;
		}
                if( tp->fast_mptcp_plus.rtt_min == 0 || tp->fast_mptcp_plus.rtt < tp->fast_mptcp_plus.rtt_min ){
                        tp->fast_mptcp_plus.rtt_min = tp->fast_mptcp_plus.rtt;
                }
		printk("sandybpf cur_rtt %u prev_rtt %u and avg_rtt %u",tp->fast_mptcp_plus.rtt,tp->fast_mptcp_plus.rtt_prev,tp->fast_mptcp_plus.rtt_avg);
        }
	if(cur_time-tp->fast_mptcp_plus.rtt_start_time>1000500){
		tp->fast_mptcp_plus.rtt_start_time=cur_time;
		tp->fast_mptcp_plus.rtt_prev=tp->fast_mptcp_plus.rtt;
	}
	if((tp->fast_mptcp_plus.rtt_avg/1000)>=(tp->fast_mptcp_plus.rtt_min*sysctl_fast_mptcp_plus_rsv1)/10000 && (!tp->fast_mptcp_plus.rsv1)&& tp->fast_mptcp_plus.tput>0){
                        tp->fast_mptcp_plus.rsv1=1;
			printk("sandybpf:path id %u Condition %d to put cap has enabled at avg_tput %d  Mbps and spont tput %d minRTT %u and avg_RTT %u \n",tp->fast_mptcp_plus.rsv2,tp->fast_mptcp_plus.rsv1,tp->fast_mptcp_plus.tput,tp->fast_mptcp_plus.tput_est,tp->fast_mptcp_plus.rtt_min,tp->fast_mptcp_plus.rtt_avg);
        }
	//End of FAST_MPTCP
